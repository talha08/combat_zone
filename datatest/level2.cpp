...
bool level2()
{
    ...
    int plife=3, enlife[3]={4, 4, 7};
    int fb =0, input, bo=0;
    player.xl= 20, player.yt=200, player.xr=100, player.yb=300;
    enemy[0].xl=550, enemy[0].yt=50, enemy[0].xr=620, enemy[0].yb=120;
    enemy[1].xl=550, enemy[1].yt=400, enemy[1].xr=620, enemy[1].yb=470;
    ...
    readimagefile("enemy1.jpg", enemy[0].xl, enemy[0].yt, enemy[0].xr, enemy[0].yb);
    readimagefile("enemy2.jpg", enemy[1].xl, enemy[1].yt, enemy[1].xr, enemy[1].yb);
    readimagefile("enemy3.jpg", enemy[2].xl, enemy[2].yt, enemy[2].xr, enemy[2].yb);
    while(1)
    {
       scoring(0);
      //p_life(plife);
        for(int i =0; i<3; i++)
        {
            ...
            if(enshot[i].xl<=0 || fb==i)
            {
                ...
                if(i<2)
                {
                    ...
                    enshot[i].yb = enemy[i].yb - 30;
                    enshot[i].yt = enemy[i].yt + 30;
                }
                else
                {
                    enshot[i].xl = enemy[i].xl - 30;
            ...
            if(enemy[i].yb> 600)
            {
                if(i==2)enemy[i].yt =0, enemy[i].yb = 80;
                else enemy[i].yt =0, enemy[i].yb = 70;
                readimagefile("background.jpg", 0, 0, 800, 600);
            }
            sprintf(name, "enemy%d.jpg", i);
  ...
  /*           else if(input == 13)
...

        {
         ...
         if(enlife[0] ==0 && enlife[1] == 0 && enlife[2] ==0)
        {
